...
printUsage()
{
  ...
  fprintf( stderr, "  Example: gethttp https://www.cs.purdue.edu/\n");
}
...
main( int argc, char ** argv )
{
  ...
  if ( !strcmp(option,"") ) {
	  ...
	  // Eliminate tags
	  int n;
	  std::cout<<"Got to load the command"<<std::endl;
	  char * buffer = fetchHTML( url, &n);
	  std::cout<<"Webpage loaded"<<std::endl;
	  if (buffer==NULL) {
	  ...
	  htmlContent.parse(buffer, n, url);
	  string spaceContainedContent = htmlContent.getSiteContent();
	  
	  std::cout<<std::endl<<"Site Content: "<<htmlContent.getSiteDescription()<<std::endl;
	  ...
	  htmlHrefs.parse(buffer, n, url);
	  vector<string>urlList = htmlHrefs.getSiteUrlList();
	  for(std::vector<string>::iterator itr = urlList.begin(); itr!= urlList.end() ;itr++){
